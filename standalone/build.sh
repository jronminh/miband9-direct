...
aapt2 link -o build/base.apk -I "$ANDROID_JAR" --manifest AndroidManifest.xml \
  build/res.zip --java build/gen --min-sdk-version 26 --target-sdk-version 33 \
...
if [ ! -f keystore.jks ]; then
  keytool -genkeypair -keystore keystore.jks -alias mibandstandalone -keyalg RSA -keysize 2048 \
    -validity 10000 -storepass mibandstandalone -keypass mibandstandalone -dname "CN=miband standalone"
fi
zipalign -f 4 build/base.apk build/aligned.apk
apksigner sign --ks keystore.jks --ks-pass pass:mibandstandalone --key-pass pass:mibandstandalone \
  --out mibandstandalone.apk build/aligned.apk
apksigner verify mibandstandalone.apk && echo "OK standalone/mibandstandalone.apk"
